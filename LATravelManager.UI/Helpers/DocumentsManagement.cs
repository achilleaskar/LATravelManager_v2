    ...
    public partial class DocumentsManagement : IDisposable
    {
        ...
        public void PrintOptionals(List<CustomerOptional> sofia)
        {
            ...
            using (GenericRepository Context = new GenericRepository())
            {
                if (sofia.Count > 0)
                {
                    FileInfo fileInfo = new FileInfo(AppDomain.CurrentDomain.BaseDirectory + @"Sources\proairetikon.xlsx");
        ...
        internal async Task PrintList(List<Booking> bookings, DateTime checkIn, DateTime checkout, bool going, Bus bus = null, bool borderonly = false)
        {
            IEnumerable<Booking> more = await Context.GetAllBookinsFromCustomers(checkIn, checkout);
            ...
            foreach (Booking booking in bookings)
            {
                ...
                {
                    if (bus == null && (r.OnlyStay || (r.Booking.SecondDepart && going)))
                    {
            ...
            foreach (var r in reservationsThisDay)
            {
                r.Starttime = GetStartingPLaceTime(r, going);
            }
            if (going)
            {
            ...
            else
            {
                ...
                {
                    if (!borderonly)
                        wbPath = GetPath($"\\Λίστα Λεωφορείου από {reservationsThisDay[0].Booking.Excursion.Destinations[0].Name} " + bus.TimeReturn.ToString("dd_MM_yy_") + bus.Vehicle.Name.Trim(new[] { '-' }));
                    else
                        wbPath = GetPath($"\\Λίστα Συνόρων από {reservationsThisDay[0].Booking.Excursion.Destinations[0].Name} " + bus.TimeReturn.ToString("dd_MM_yy_") + bus.Vehicle.Name.Trim(new[] { '-' }));
                }
                else
                {
                    if (!borderonly)
                        wbPath = GetPath($"\\Λίστα Λεωφορείου από {reservationsThisDay[0].Booking.Excursion.Destinations[0].Name} " + checkout.ToString("dd_MM_yy"));
                    else
                        wbPath = GetPath($"\\Λίστα Συνόρων από {reservationsThisDay[0].Booking.Excursion.Destinations[0].Name} " + checkout.ToString("dd_MM_yy"));
                }
            }
            ...
            if (going)
                ...
                myWorksheet.Cells["A1"].Value = "Επιστροφή - " + (bus == null ? checkout.ToString("dd/MM/yyyy") : bus.TimeReturn.ToString("dd/MM/yyyy"));
            ...
            if (reservationsThisDay.Count > 0)
            {
                foreach (ReservationWrapper reservation in reservationsThisDay)
                {
                    ...
                    foreach (Customer customer in reservation.CustomersList)
                    {
                        ...
                        {
                            if ((bus == null || (!bus.Going && customer.BusReturn != null && customer.BusReturn.Id == bus.Id)) && customer.CustomerHasBusIndex % 2 == 0 && (!reservation.Booking.DifferentDates || customer.CheckOut == checkout))
                            {
                                counter++;
                                myWorksheet.InsertRow(lineNum, 1);
                                customersCount++;
                                ...
                                if (!borderonly)
                                {
                                    myWorksheet.Cells["F" + lineNum].Value = customer.ReturningPlace;
                                    if (customer.Tel != null && (customer.Tel.StartsWith("69", StringComparison.Ordinal) || customer.Tel.StartsWith("+", StringComparison.Ordinal) || (customer.Tel.StartsWith("00", StringComparison.Ordinal) && customer.Tel.Length > 10 && customer.Tel.Length < 16)))
                                    {
                                        myWorksheet.Cells["G" + lineNum].Value = customer.Tel;
                                        PhoneNumbers += customer.Tel + ",";
                ...
                MessageBox.Show("Δέν υπάρχουν άτομα που να πηγαίνουν τις συγκεκριμένες ημέρες από τις επιλεγμένες πόλεις", "Σφάλμα");
            if (!borderonly && bus != null && bookings[0].Excursion.Id != 2)
            {
        ...
        private DateTime GetStartingPLaceTime(ReservationWrapper r1, bool going)
        {
            DateTime r = DateTime.Today;
            try
            {
                if (going)
                    return (new DateTime() + r1.Booking.Excursion.Destinations[0].ExcursionTimes.FirstOrDefault(t => t.StartingPlace.Name == r1.CustomersList[0].StartingPlace).Time).AddHours(9).AddSeconds(r1.CustomersList[0].StartingPlace[0]);
                return (new DateTime() + r1.Booking.Excursion.Destinations[0].ExcursionTimes.FirstOrDefault(t => t.StartingPlace.Name == r1.CustomersList[0].ReturningPlace).Time).AddHours(9).AddSeconds(r1.CustomersList[0].ReturningPlace[0]);
            }
            catch
            {
            }

            return new DateTime().AddHours(23);
        }
